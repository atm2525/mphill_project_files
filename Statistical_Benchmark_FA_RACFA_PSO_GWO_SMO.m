...
Benchmarks = {
    'Ackley',       -32,     32,     30, 0, 1e-5;
    'Rastrigin',    -5.12,   5.12,   30, 0, 1e-5;
    'Schwefel',    -500,    500,      2, 0, 1e-5;
    'Michalewicz',   0,      pi,     10, -9.66015, 1e-5;
    'Rosenbrock',   -5,      10,     30, 0, 1e-1;
};
...
switch lower(fname)
    ...
    case 'schwefel'
        ...
        fobj = @(x) ...
            418.9829*dim - sum(x.*sin(sqrt(abs(x))));
    ...
    case 'michalewicz'
        ...
        fobj = @(x) ...
            -sum(sin(x) .* (sin(ii .* x.^2 ./ pi)).^(2*m));

    case 'rosenbrock'

        fobj = @(x) ...
            sum(100*(x(2:end)-x(1:end-1).^2).^2 ...
            + (x(1:end-1)-1).^2);
